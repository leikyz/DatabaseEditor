...
public sbyte questType;
        ...
        public IEnumerable<TreasureHuntFlag> flags;
...
public TreasureHuntMessage(sbyte questType, int startMapId, IEnumerable<Types.TreasureHuntStep> knownStepsList, sbyte totalStepCount, uint checkPointCurrent, uint checkPointTotal, int availableRetryCount, IEnumerable<TreasureHuntFlag> flags)
        {
...
questType = reader.ReadSByte();
            ...
            {
                 (knownStepsList as Types.TreasureHuntStep[])[i] = ProtocolTypeManager.GetInstance<Types.TreasureHuntStep>(reader.ReadShort());
            ...
            flags = new TreasureHuntFlag[limit];
            for (int i = 0; i < limit; i++)
            {
                
                    ((TreasureHuntFlag[])flags)[i] = new TreasureHuntFlag();
                    ((TreasureHuntFlag[])flags)[i].Deserialize(reader);
